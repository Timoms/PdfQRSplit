...
parser = argparse.ArgumentParser(
    description='Split PDF-file into separate files based on a separator barcode')
...
parser.add_argument('-o', '--output', default="./",
                    help='Output path for generated files. Default: ./')
...
parser.add_argument('-rm', '--remove-original',
                    help='Remove original file after splitting', default=False)
parser.add_argument('-k', '--keep-page', action='store_true',
                    help='Keep separator page in previous document')
parser.add_argument('-kn', '--keep-page-next', action='store_true',
    ...
    def split_qr(self, split_text: str, ifiles: int) -> int:
        ...
        while current_page != self.total_pages:
            ...
            with TemporaryDirectory() as temp_dir:
                if self.debug:
                    print(
                        "    Writing page images to temporary directory {}".format(temp_dir))
                ...
                for obj in xObject:
                    ...
                    if xObject[obj]['/Subtype'] == '/Image':
                        ...
                        if xObject[obj]['/Filter'] == '/FlateDecode':
                            ...
                            def fn(x): return 255 if x > 128 else 0
                        ...
                        if tgtn:
                            if self.debug:
                                print(
                                    "      Wrote image {}; Checking for separator barcode".format(tgtn))
                ...
                if split:
                    ...
                    output = args.output + args.prefix + '_' + \
                        str(ifiles) + '_' + str(pdfs_count) + '.pdf'
                    if self.verbose:
                        print(
        ...
        if self.verbose:
            print(
                "    End of input - writing {} pages to {}".format(pdf_writer.getNumPages(), output))
        with open(output, 'wb') as output_pdf:
            pdf_writer.write(output_pdf)
        pdfs_count += 1
